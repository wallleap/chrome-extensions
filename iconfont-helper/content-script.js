...
function addCheckboxes() {
  // 尝试多种选择器来找到图标项
  const iconListItems = document.querySelectorAll(
    ".block-icon-list > li, .icon-list > li, .block-icon",
  );
  iconListItems.forEach((item) => {
    ...
    if (!item.querySelector(".iconfont-helper-checkbox")) {
      ...
      checkbox.addEventListener("change", function () {
        ...
        // 尝试通知 popup 页面更新选择状态
        chrome.runtime
          .sendMessage({ type: "selection-changed" })
  ...
  selectedItems.forEach((item, index) => {
    ...
    if (svgElement) {
      ...
      if (svgNameList.includes(fileName)) {
        const count = svgNameList.filter(
          (svgName) => svgName === fileName,
        ).length;
        fileName += count;
      }
      fileName += ".svg";
